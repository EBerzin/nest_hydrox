...
int main(int argc, char** argv) {
  ...
  // INTERACTION_TYPE type_num = NEST::beta;
    ...
    NRYieldsParam.push_back(11.);  // alpha, for NR model. See http://nest.physics.ucdavis.edu
    ...
    NRYieldsParam.push_back(1.0);  // makes low-E sigmoid an asymmetric one, for charge
    NRYieldsParam.push_back(1.0);  // makes low-E sigmoid an asymmetric one, for light
  ...
  for (uint64_t j = 0; j < numEvts; ++j) {
    ...
    vD = vTable[index];
    driftTime =
        (detector->get_TopDrift() - pos_z) / vD;   
    ...
    yields = n.GetYields(type_num, keV, rho, field, double(massNum), double(atomNum),
                  NRYieldsParam, default_ERYieldsParam);
  ...
  // Print selected outputs a la execNEST
   ...
   //   printf("%.6f\t%.6f\t%.6f\t", scint[2], scint[5],
    ...
    //}
     ...
     //fprintf(
	   // stdout,
	   // "\nE [keV]\t\tfield [V/cm]\ttDrift [us]\tX,Y,Z [mm]\tNph\tNe-\n");
      cout << keV << " " << field  << " " << driftTime  << " " << truthPos[0]  << " " << 
        truthPos[1] << " " << truthPos[2]  << " "  << yields.Lindhard << " " << yields.PhotonYield  << " " << yields.ElectronYield << endl;
      //printf("%.6f\t%.6f\t%.6f\t%.0f, %.0f, %.0f\t%.6f\t%.6f\t", keV, field, driftTime,
	   //truthPos[0], truthPos[1], truthPos[2], yields.PhotonYield,
